...
export AIO_TOOL="gemini"

_aio_ensure_registered() {
    if [[ ! -f "$HOME/.ai-orchestrator/.current_instance" ]] || \
       ! grep -q "^gemini-" "$HOME/.ai-orchestrator/.current_instance" 2>/dev/null; then
        aio register gemini "$(pwd)"
    fi
}
...
echo "Gemini AI Orchestrator integration loaded"
